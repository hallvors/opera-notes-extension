...
function createInsertNoteMenu () {
	chrome.contextMenus.removeAll(function(){
		insertNoteMenu = chrome.contextMenus.create({'title':'Insert Note', 'contexts':['editable']});
		datamanager.loadData(
			function(){
				datamanager.iterate(null, function (obj, parentAr, idx, parentObj) { // Its first argument is the object, second is parent array, third index, fourth is the object that referenced the array
					if(obj.trash_folder || (parentObj && parentObj.trash_folder))return; // Trashed notes should not appear in menu
...
createInsertNoteMenu();

chrome.extension.onMessage.addListener(function(msg){
	if(msg.update_insert_note_menu)createInsertNoteMenu();
});
